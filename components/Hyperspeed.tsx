...
const Hyperspeed = ({ effectOptions = {
  ...
  carFloorSeparation: [0, 5],
      colors: {
      roadColor: 0x1a1a1a, // 道路颜色（深灰色，适合磨砂效果）
      islandColor: 0x0f0f0f, // 分隔岛颜色（更深的灰色）
      background: 0x000000, // 保持黑色背景
      shoulderLines: 0xFFFFFF,
      brokenLines: 0xFFFFFF,
      leftCars: [0xD856BF, 0x6750A2, 0xC247AC], // 保持洋红色车灯
      rightCars: [0x03B3C3, 0x0E5EA5, 0x324555], // 保持青色车灯
      sticks: 0x03B3C3, // 保持青色光条
    }
} }) => {
  ...
  useEffect(() => {
    ...
    class App {
      ...
      private road: any;
      private leftCarLights: any;
      private rightCarLights: any;
      private leftSticks: any;
    ...
    class CarLights {
      ...
      init() {
        ...
        (instanced as any).copy(geometry);
    ...
    class Road {
      ...
      private options: any;
      private uTime: any;
      private leftRoadWay: any;
      private rightRoadWay: any;
      private island: any;
      ...
      createPlane(side: number, width: number, isRoad: boolean) {
        ...
        let uniforms = {
          ...
          uTime: this.uTime,
          uOpacity: { value: isRoad ? 0.3 : 0.2 } // 道路半透明，分隔岛更透明
        };
        ...
        const material = new THREE.ShaderMaterial({
          ...
          side: THREE.DoubleSide,
          transparent: true,
          opacity: isRoad ? 0.3 : 0.2, // 道路半透明，分隔岛更透明
          uniforms: Object.assign(
            uniforms,
            this.webgl.fogUniforms,
            options.distortion.uniforms
    ...
    const roadBaseFragment = `
      ...
      void main() {
        ...
        // 添加磨砂效果
        float noiseValue = fbm(uv * 20.0 + uTime * 0.1);
        float roughness = 0.1; // 磨砂强度
        color = mix(color, color * (0.8 + noiseValue * 0.4), roughness);
        
        #include <roadMarkings_fragment>
        
        // 应用磨砂透明度
        float finalOpacity = uOpacity * (0.7 + noiseValue * 0.3);
    ...
    const islandFragment = `
      ...
      void main() {
        ...
        // 添加磨砂效果（分隔岛稍微柔和一些）
        float noiseValue = fbm(uv * 15.0 + uTime * 0.05);
        float roughness = 0.08; // 磨砂强度稍低
        color = mix(color, color * (0.85 + noiseValue * 0.3), roughness);
        
        // 应用磨砂透明度
        float finalOpacity = uOpacity * (0.6 + noiseValue * 0.4);
        gl_FragColor = vec4(color, finalOpacity);
        ${THREE.ShaderChunk["fog_fragment"]}
      }
    `;
    ...
    function resizeRendererToDisplaySize(renderer: any, setSize: (width: number, height: number, updateStyles?: boolean) => void) {
